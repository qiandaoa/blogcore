...
namespace Blog.Core.Repository.sugar
{
    public class DbContext
    {
        ...
        private DbContext() 
        {
            if (string.IsNullOrEmpty(_connectionString)) // IsNullOrEmpty检查这个属性是空的还是null
            {
                throw new ArgumentNullException("数据库连接字符串为空"); // ArgumentNullException表示收到一个null会抛出一个异常
            }
